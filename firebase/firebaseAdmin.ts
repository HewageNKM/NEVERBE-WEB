...
export const getReviewByItemId = async (itemId: string, userId: string) => {
  try {
    ...
    const totalDocs = await adminFirestore
      .collection("reviews")
      .where("itemId", "==", itemId)
      .get();
    const totalReviews = totalDocs.size;
    const totalRating = calculateTotalRating(
      docs.docs.map((doc) => doc.data() as Review)
    );

    // Check if the user has reviewed the item
    const isUserReviewed = docs.docs.some(
      (doc) => doc.data().userId === userId && doc.data().itemId === itemId
    );
    ...
    if (!userReviewSnapshot.empty) {
      console.log(
        `User review found where itemId == ${itemId} and userId == ${userId}`
      );
      ...
      console.log(
        `User review not found where itemId == ${itemId} and userId == ${userId}`
      );
    }
    ...
    console.log(
      `Total reviews fetched where itemId == ${itemId}:`,
      reviews.length
    );
...
export const sendEmail = async (msg: Message, token: string) => {
  try {
    console.log(
      `Sending email to: ${msg.email}, with subject: ${msg.subject}, and message: ${msg.message}`
    );
    const res = await verifyCaptchaToken(token);
    if (!res) {
      throw new Error("reCAPTCHA verification failed.");
