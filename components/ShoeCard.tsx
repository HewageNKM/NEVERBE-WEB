...
const ShoeCard = ({shoe, type}: {
    ...
    const setAvailableSizes = (size: number, index: number) => {
        ...
        return <motion.button initial={{opacity: 0, y: "1vh"}}
                              animate={{opacity: `${stocks[size] <= 0 ? '0.5' : '1'}`, y: 0}}
                              transition={{duration: 0.5}} exit={{opacity: 0, y: '1vh'}} key={index} onClick={() => {
            setSelectedSize(size)
        }}
                              className={`capitalize p-2 rounded-md ${selectedSize === size ? 'bg-primary text-white' : 'bg-gray-200 text-black'}}`}
                              disabled={stocks[size] <= 0}>{size}</motion.button>
    }
    const checkStocksAvailability = () => {
        const stocks = shoe.stocks;
        let count = 0;
    ...
    return (
        <div onMouseEnter={() => setIsMouseOver(true)}
            ...
            <AnimatePresence>
                {addToCart && (
                    <motion.div initial={{y: "1vh", opacity: 0}} animate={{y: 0, opacity: 1}}
            ...
            {outOfStock && (<div
                className="bg-white rounded-lg shadow-custom font-semibold text-black absolute top-1 right-1 text-sm w-fit p-1">Out
                of
                Stock
            </div>)
            }
