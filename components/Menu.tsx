...
const Menu = () => {
    ...
    const onSearch = async (evt) => {
        ...
        try {
            ...
            const filteredResults = await searchResults.results[0].hits.filter((item) => (item.status === "Active" && item.listing == "Active"))
    ...
    return (
        <DropShadow containerStyle="fixed inset-0 flex justify-end items-center bg-black/50">
            <motion.div
                ...
                className="w-[90vw] h-full bg-white rounded-l-sm shadow-lg relative flex flex-col  overflow-y-auto"
            >
                ...
                {(showSearchResult && items.length > 0) && <SearchDialog
